...
export default function LiveTradingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#111] text-[#e8e8e8] pb-12">
      {/* Header */}
      <header className="border-b border-[var(--ax-border)] bg-[#1a1a1a]/50 backdrop-blur sticky top-0 z-50">
      ...
      {notif && (
        <div className={`fixed top-16 right-4 z-50 px-4 py-3 rounded-lg shadow-lg border text-sm max-w-sm ${
          ...
          "bg-[#111]/90 border-[#2a3a5a] text-[#93b4ff]"
        }`}>
          {notif.msg}
        </div>
      )}
      ...
      {showTotpModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl p-6 w-full max-w-sm mx-4 shadow-2xl">
            ...
            <input
              ...
              placeholder="000000"
              className="w-full bg-[#1a1a1a] border border-[var(--ax-border)] rounded-lg px-3 py-2 text-center text-lg tracking-[0.5em] font-mono focus:outline-none focus:border-[#4a7fcc] mb-4"
              autoFocus
              onKeyDown={(e) => {
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex-1 px-4 py-2 bg-[#4a7fcc] hover:bg-[#5a8fd0] disabled:opacity-50 rounded-lg text-sm font-medium transition-colors"
              >
                {authPending ? "Connecting..." : "Connect"}
              </button>
              <button
                onClick={() => setShowTotpModal(false)}
                className="px-4 py-2 bg-[#1a1a1a] hover:bg-[#222] rounded-lg text-sm font-medium transition-colors"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Deployment Selector */}
          <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl p-4 flex flex-col justify-between">
            <div>
              <label className="text-xs text-[#a0a0a0] uppercase tracking-wider mb-2 block font-semibold">Deployment</label>
              <select
                value={selectedDeploymentId}
                onChange={(e) => setSelectedDeploymentId(e.target.value)}
                className="w-full bg-[#1a1a1a] border border-[var(--ax-border)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#4a7fcc]"
              >
          ...
          <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl p-4 flex flex-col justify-between">
            <div>
              <label className="text-xs text-[#a0a0a0] uppercase tracking-wider mb-2 block font-semibold">Starting Capital Config</label>
              <div className="relative">
                <span className="absolute left-3 top-2.5 text-[#606060] text-xs font-semibold">₹</span>
                <input
                  type="text"
                  value={capitalInput}
                  onChange={(e) => setCapitalInput(e.target.value.replace(/\D/g, ""))}
                  placeholder="100000"
                  className="w-full bg-[#1a1a1a] border border-[var(--ax-border)] rounded-lg pl-7 pr-3 py-2 text-sm focus:outline-none focus:border-[#4a7fcc] font-semibold"
                />
              </div>
            </div>
            <button
              ...
              className="mt-4 w-full flex items-center justify-center gap-2 px-4 py-2 bg-[#1a1a1a] hover:bg-[#222] border border-[var(--ax-border)] disabled:opacity-50 rounded-lg text-sm font-semibold transition-colors"
            >
          ...
          {/* Active Asset Info */}
          <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-[#a0a0a0] font-semibold">{status?.interval || "—"} • Step {status?.step ?? 0}</div>
              </div>
            </div>
            <div className="text-right">
              <div className="text-xl font-mono font-bold text-[#e8e8e8]">
              ...
              <div className="flex items-center justify-end gap-1.5 mt-1">
                {status?.smartapi_connected ? (
                  marketDataActive || status?.market_data_active ? (
                    ...
                    <span className="flex items-center gap-1 text-[10px] text-[#606060] font-bold bg-[#1a1a1a] border border-[var(--ax-border)] px-1.5 py-0.5 rounded">
        ...
        {status?.portfolio && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl p-4 shadow-lg">
              <div className="text-xs text-[#606060] uppercase tracking-wider mb-1 font-bold">Equity</div>
              <div className="text-2xl font-bold text-[#e8e8e8] font-mono">₹{status.portfolio.equity.toLocaleString("en-IN", { maximumFractionDigits: 2 })}</div>
              <div className={`text-xs mt-1.5 font-semibold ${pnlChange >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                {pnlChange >= 0 ? "+" : ""}₹{pnlChange.toFixed(2)}
              </div>
            </div>
            <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl p-4 shadow-lg">
              <div className="text-xs text-[#606060] uppercase tracking-wider mb-1 font-bold">Total PnL</div>
              ...
              <div className="text-xs text-[#606060] mt-2 font-medium">Realized + Unrealized</div>
            </div>
            <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl p-4 shadow-lg">
              <div className="text-xs text-[#606060] uppercase tracking-wider mb-1 font-bold">Cash Balance</div>
              <div className="text-2xl font-bold text-[#e8e8e8] font-mono">₹{status.portfolio.cash.toLocaleString("en-IN", { maximumFractionDigits: 2 })}</div>
              <div className="text-xs text-[#606060] mt-2 font-medium">Available to Trade</div>
            </div>
            <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl p-4 shadow-lg">
              <div className="text-xs text-[#606060] uppercase tracking-wider mb-1 font-bold">Taxes & Charges</div>
              <div className="text-2xl font-bold text-amber-500 font-mono">₹{status.portfolio.total_fees.toFixed(2)}</div>
              <div className="text-xs text-[#606060] mt-2 font-medium">Brokerage + STT + GST</div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl overflow-hidden shadow-2xl p-4">
              <h3 className="text-sm font-semibold text-[#c0c0c0] mb-3 flex items-center gap-2">
              ...
              {candles.length > 0 ? (
                ...
                <div className="h-[420px] bg-[#111] rounded-lg flex flex-col items-center justify-center border border-[var(--ax-border)] text-[#606060] text-sm gap-3">
                  {selectedDeploymentId ? (
                    <>
                      ...
                      <p className="text-xs text-[#606060] max-w-md text-center px-4">
                      ...
                      <div className="text-[10px] text-[#a0a0a0] mt-2">
          ...
          <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl p-5 shadow-2xl space-y-4">
            <h3 className="font-bold text-[#c0c0c0] text-sm flex items-center gap-2 border-b border-[var(--ax-border)] pb-3">
              <PlusCircle className="w-5 h-5 text-[#93b4ff]" />
            ...
            <form onSubmit={handleExecuteManualTrade} className="space-y-4">
              {/* Asset Display */}
              <div>
                <label className="block text-[10px] uppercase font-bold text-[#606060] mb-1">Asset Symbol</label>
                <div className="w-full bg-[#111] border border-[var(--ax-border)] rounded px-3 py-2 text-[#a0a0a0] font-bold text-xs">
              ...
              {/* Buy/Sell Direction */}
              <div>
                <label className="block text-[10px] uppercase font-bold text-[#606060] mb-1">Direction</label>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`py-1.5 rounded text-xs font-bold transition-all ${
                      manualDirection === "BUY"
                        ? "bg-emerald-600 text-[#f0f0f0] shadow"
                        : "bg-[#111] text-[#a0a0a0] hover:text-[#c0c0c0] border border-[var(--ax-border)]"
                    }`}
                  >
                    BUY
                  </button>
              ...
              {/* Quantity */}
              <div>
                <label className="block text-[10px] uppercase font-bold text-[#606060] mb-1">Quantity (Qty)</label>
                <input
                  type="number"
                  min={1}
                  value={manualQty}
                  onChange={(e) => setManualQty(Math.max(1, parseInt(e.target.value) || 1))}
                  className="w-full text-xs bg-[#111] border border-[var(--ax-border)] rounded px-3 py-2 text-[#c0c0c0] focus:outline-none focus:border-[#4a7fcc] font-semibold font-mono"
                />
              </div>
                  ...
                  <button
                    ...
                    className={`py-1.5 rounded text-xs font-bold transition-all ${
                      manualOrderType === "MARKET"
                        ? "bg-[#1a1a1a] text-[#93b4ff] border border-[#2a3a5a]"
                        : "bg-[#111] text-[#a0a0a0] border border-[var(--ax-border)]"
                    }`}
                  >
                    MARKET
                  </button>
              ...
              {manualOrderType === "LIMIT" && (
                <div>
                  <label className="block text-[10px] uppercase font-bold text-[#606060] mb-1">Limit Price (₹)</label>
                  <input
                    type="text"
                    value={manualPrice}
              ...
              <button
                ...
                className={`w-full text-[#f0f0f0] font-bold text-xs py-2 rounded transition-all flex items-center justify-center gap-2 ${
                  manualDirection === "BUY"
                    ? "bg-emerald-600 hover:bg-emerald-700 disabled:bg-[#1a1a1a] disabled:text-[#606060]"
                    : "bg-red-600 hover:bg-red-700 disabled:bg-[#1a1a1a] disabled:text-[#606060]"
                }`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Positions Table */}
            <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl overflow-hidden shadow-lg">
              <div className="px-4 py-3 border-b border-[var(--ax-border)] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Wallet className="w-4 h-4 text-emerald-400" />
                  <h3 className="font-semibold text-sm">Active Positions</h3>
                </div>
                <span className="text-xs text-[#606060] font-semibold">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="text-[#a0a0a0] border-b border-[var(--ax-border)] text-left bg-[#111]/40">
                  ...
                  <tbody>
                    {Object.keys(status?.portfolio?.positions || {}).length === 0 && (
                      <tr>
                        <td colSpan={5} className="px-4 py-6 text-center text-[#606060] font-medium">
                    ...
                    {Object.entries(status?.portfolio?.positions || {}).map(([sym, pos]: [string, any]) => {
                      ...
                      return (
                        <tr key={sym} className="border-b border-[var(--ax-border)]/40 hover:bg-[#1a1a1a]/40 font-mono">
                          <td className="px-4 py-2.5 font-bold text-[#c0c0c0]">{sym}</td>
                          ...
                          <td className="px-4 py-2.5 text-right text-[#a0a0a0]">₹{curVal.toFixed(2)}</td>
            ...
            <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl overflow-hidden shadow-lg">
              <div className="px-4 py-3 border-b border-[var(--ax-border)] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Activity className="w-4 h-4 text-[#93b4ff]" />
                  <h3 className="font-semibold text-sm">Recent Trades & Fees</h3>
                </div>
                <span className="text-xs text-[#606060] font-semibold">{trades.length} trades</span>
              </div>
              <div className="overflow-x-auto max-h-72 custom-scrollbar">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="text-[#a0a0a0] border-b border-[var(--ax-border)] text-left bg-[#111]/40 sticky top-0">
                  ...
                  <tbody>
                    {trades.length === 0 && (
                      <tr>
                        <td colSpan={7} className="px-4 py-6 text-center text-[#606060] font-medium">
                    ...
                    {trades.map((trade) => {
                      ...
                      return (
                        <tr key={trade.id} className="border-b border-[var(--ax-border)]/40 hover:bg-[#1a1a1a]/40 font-mono">
                          <td className="px-4 py-2 text-[#606060]">{displayTime}</td>
                          <td className="px-4 py-2 font-bold text-[#c0c0c0]">{trade.symbol}</td>
                          ...
                          <td className="px-4 py-2 text-right text-[#a0a0a0]">₹{trade.price.toFixed(2)}</td>
                          <td className="px-4 py-2 text-right text-[#a0a0a0]">₹{trade.value.toFixed(2)}</td>
          ...
          <div className="space-y-6">
            {/* Event Logs */}
            <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl overflow-hidden shadow-lg flex flex-col h-[320px]">
              <div className="px-4 py-3 border-b border-[var(--ax-border)] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <RefreshCw className="w-4 h-4 text-purple-400" />
                  <h3 className="font-semibold text-sm">Deployment Events</h3>
                </div>
                <span className="text-xs text-[#606060] font-mono font-semibold">{events.length}</span>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                {events.length === 0 && (
                  <div className="text-center text-[#606060] text-xs py-8">
                ...
                {events.map((event) => {
                  ...
                  return (
                    <div key={event.id} className="text-xs leading-relaxed border-b border-[var(--ax-border)] pb-2">
                      <div className="flex items-start gap-2">
                        ...
                        {event.event_type === "start" && <Play className="w-3.5 h-3.5 text-[#93b4ff] mt-0.5 shrink-0" />}
                        {event.event_type === "stop" && <Square className="w-3.5 h-3.5 text-[#a0a0a0] mt-0.5 shrink-0" />}
                        {!["fill", "error", "margin_call", "start", "stop"].includes(event.event_type) && (
                          <Activity className="w-3.5 h-3.5 text-[#a0a0a0] mt-0.5 shrink-0" />
                        )}
                        <div className="flex-1">
                          <div className="text-[#a0a0a0] font-medium">{event.message}</div>
                          <div className="text-[10px] text-[#606060] font-mono mt-0.5">{displayTime}</div>
                        </div>
                      </div>
                    </div>
                  );
            ...
            {trades.length > 0 && (
              <div className="bg-[#1a1a1a] border border-[var(--ax-border)] rounded-xl overflow-hidden shadow-lg">
                <button
                  ...
                  className="w-full px-4 py-3 flex items-center justify-between hover:bg-[#1a1a1a]/50 transition-colors border-b border-[var(--ax-border)]"
                >
                  <div className="flex items-center gap-2">
                ...
                {showChargesDetail && (
                  <div className="p-4 space-y-2.5 font-mono text-xs">
                    {(() => {
                      ...
                      return (
                        <>
                          <div className="flex justify-between"><span className="text-[#a0a0a0]">Brokerage</span><span>₹{totals.brokerage.toFixed(2)}</span></div>
                          <div className="flex justify-between"><span className="text-[#a0a0a0]">STT / CTT</span><span>₹{totals.stt.toFixed(2)}</span></div>
                          <div className="flex justify-between"><span className="text-[#a0a0a0]">Exchange Charges</span><span>₹{totals.exc.toFixed(2)}</span></div>
                          <div className="flex justify-between"><span className="text-[#a0a0a0]">GST (18%)</span><span>₹{totals.gst.toFixed(2)}</span></div>
                          <div className="flex justify-between"><span className="text-[#a0a0a0]">SEBI Turnovers</span><span>₹{totals.sebi.toFixed(2)}</span></div>
                          <div className="flex justify-between"><span className="text-[#a0a0a0]">Stamp Duty</span><span>₹{totals.stamp.toFixed(2)}</span></div>
                          <div className="border-t border-[var(--ax-border)] pt-2 flex justify-between font-bold text-sm">
