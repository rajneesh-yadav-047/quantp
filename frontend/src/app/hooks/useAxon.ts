...
export function useAxon() {
  ...
  // ── Clear selected strategy if it no longer exists in the list ──
  useEffect(() => {
    if (selectedStrategyId && strategies.length > 0) {
      const stillExists = strategies.some((s: any) => s.id === selectedStrategyId);
      if (!stillExists) {
        setSelectedStrategyId("");
        setSelectedRunId("");
        setBacktestDetail(null);
        setReplayEvents([]);
        triggerNotif("info", "Previously selected strategy was removed. Selection cleared.");
      }
    }
  }, [strategies, selectedStrategyId, triggerNotif]);
  ...
  // ── OPTIONS BACKTEST WITH COVERAGE CHECK ──
      ...
      if (res.ok && res.data) {
        ...
        if (job.status === "completed") {
          ...
          if (stateRef.current.pendingBacktest) {
            ...
            const covers = pb.symbols.some(s => s.toUpperCase() === (job.symbol || "").toUpperCase()) && pb.interval.toUpperCase() === (job.interval || "").toUpperCase();
            if (covers) {
              setTimeout(() => {
                setPendingBacktest(null);
                handleRunBacktestRef.current();
              }, 500);
            }
          }
          // ── Multi-asset auto-resume (async path) ──
          if (stateRef.current.pendingMultiAsset) {
            setTimeout(() => {
  ...
  const handleSaveStrategy = useCallback(async () => {
    ...
    const payload = {
      name: strategyName,
      description: "Strategy created in Axon",
      code,
